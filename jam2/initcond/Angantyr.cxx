...
void Angantyr::init()
{
  ...
  //auto impactParHook = make_shared<ImpactParameterHook>(bGen);
  impactParHook = make_shared<ImpactParameterHook>(bGen);
...
void Angantyr::generate(Collision* event, int mode)
{
  ...
  HeavyIonsPtr heavyIons = pythia->heavyIonsPtr;
  ...
  for(int i=0; i<pythia->event.size();i++) {
    ...
    //Pythia8::ParticleDataEntry* pa= &py.particleDataEntry();
    //ParticleDataEntryPtr ptr = make_shared<ParticleDataEntry>(*pa);
    ParticleDataEntryPtr ptr = make_shared<ParticleDataEntry>(py.particleDataEntry());

    EventParticle* cp = new EventParticle(id,m,r,p,ptr);
    ...
    cp->setOnShell();
    double e=cp->getE0();
    double dect = jamParticleData->lifeTime(ptr,m,e);
