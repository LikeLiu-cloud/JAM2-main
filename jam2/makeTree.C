...
int makeTree(std::string fname) {
   ...
   float px[MAXMULT], py[MAXMULT], pz[MAXMULT], pe[MAXMULT], rx[MAXMULT], ry[MAXMULT], rz[MAXMULT], rt[MAXMULT], Tf[MAXMULT], mass[MAXMULT];
   ...
   t1->Branch("Tf", Tf, "Tf[mulsel]/F"); // formation time in fm
   ...
   while (std::getline(ifs,line)) {
	 ...
	 //cout<<"mul = "<<mul<<endl;
         ...
         //if (abs(pidval) == 211 || abs(pidval) == 321 || pidval == 311 || pidval == 333 || abs(pidval) == 2212 || abs(pidval) == 3122  ) {
