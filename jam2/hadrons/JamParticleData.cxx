...
JamParticleData::JamParticleData(Pythia8::ParticleData* pd, Pythia8::Rndm* rnd)
{
  ...
  Mesons* m=new Mesons();
    ...
    // include Delta(1232).
    Pythia8::ParticleDataEntryPtr dm=table->findParticle(1114);
    Pythia8::ParticleDataEntryPtr d0=table->findParticle(2114);
    Pythia8::ParticleDataEntryPtr dp=table->findParticle(2214);
    Pythia8::ParticleDataEntryPtr dpp=table->findParticle(2224);
    /*
    ...
    */
    ...
    n1440   = table->findParticle(202112);
    p1440   = table->findParticle(202212);
  ...
  while (table->nextId(id) != 0 ) {
    ...
    ParticleDataEntryPtr p=table->findParticle(id);
    id=table->nextId(id);
...
void JamParticleData::addDiffractiveParticle(ParticleData* pd)
{
  ...
  pd->addParticle(10022110,"N(1650)_diff0","N(1650)_diffbar0",0, 0);
  pd->addParticle(10122110,"N(1650)_diff0","N(1650)_diffbar0",0, 0);
  ...
  pd->addParticle(9910220,"f_0_diff0","f_0_diff0",0,0);
}

double JamParticleData::lifeTime(Pythia8::ParticleDataEntryPtr pd,double m, double e)
{
  if(m<1e-10) return 1.e+35;

  // constant width.
  //double wid0 = pd->mWidth()/Pythia8::HBARC;
  ...
  if(m > 0.0) {
    ...
    double wid = decayWidth->getTotalWidth(pd,m)/Pythia8::HBARC;
    double t = wid > 1e-8 ?  -log(max(rndm->flat(), 1.e-35) ) /wid : 1e+35;

    //cout << pd->name() << " wid= " << wid << " wid0= "<< pd->mWidth()/Pythia8::HBARC <<" t= " << t <<  " gam= "<< e/m <<endl;
    //cin.get();
  ...
  double wid = decayWidthTable[pd] !=0 ?
       decayWidthTable[pd]->getWidth(m)/Pythia8::HBARC :
      decayWidth->getTotalWidth(pd,m)/Pythia8::HBARC;

  if(decayWidthTable[pd]==0)
      cout << "JamParticleData::lifetime id= "<< pd->id()<<endl;

  //double wid=decayWidthTable[pd]->getWidth(m)/Pythia8::HBARC;
  double t = wid > 1e-8 ?  -log(max(rndm->flat(), 1.e-35) ) /wid : 1e+35;
  return t * e/m;
}
