	...
	ParticleTable* m, ParticleTable* light0)
{
	...
	20333, 9010221,   9000221,100223};

    for(int i=0;i<nkf;i++) {
	ParticleDataEntryPtr pr =particleData->findParticle(kf[i]);
	//cout << " kf= "<< kf[i] << " p= " << p << endl;
	if(pr) {
	    m->add(pr); light0->add(pr);
	}
    }
    ...
    */
    ...
    p->addChannel( 1,0.3333300000,0, 213, -211);
    p->addChannel( 1,0.3333300000,0, -213, 211);
    p->addChannel( 1,0.3333300000,0, 113, 111);
    m->add(p);
    light0->add(p);
    */
	...
	ParticleTable* m, ParticleTable* light10,ParticleTable* light1p)
{
    ...
    const int kf[18]={111, 211, 113, 213, 115, 215, 10111,10211,
	//a_1(1260)0   b_1(1235)0       rho(1450)      a_0(980)         rho(1700)0
	20113, 20213,  10113, 10213,   100113,100213, 9000111, 9000211, 30113, 30213};
  ...
  // list of particle which is not set angular momentum in pythia8
  // 115 a_20 1.3183 215 a_2+ 1.3183
  // 335 f'_2(1525)
  // 10111 a_0(1450)0
  // 223 omega  331 eta' 333 phi
    // 100113 rho(1450)0
