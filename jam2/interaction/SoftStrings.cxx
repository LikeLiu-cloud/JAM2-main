...
SoftStrings::SoftStrings(Pythia8::Info* inf,Pythia8::Settings& s,JamParticleData* jd,
        CrossSection* xs,
        Pythia8::Pythia* py,
        Pythia8::StringFlav* flav,
        Pythia8::Rndm* r) :  Scatter2(inf,&s,jd,xs,r)
{
    jamParticleData=jd;
    ...
    sigtot.initInfoPtr(*inf);
    sigtot.init();
    //sigtot.init(&hadronize->info,settings0,particleData,rndm0);

    hadronContent = new HadronContent(particleData,r);
    beam1 = new BeamHadron(s,particleData,r);
    beam2 = new BeamHadron(s,particleData,r);
  ...
  epsSU6uu1 = settings->parm("BeamRemnants:epsSU6uu1");
  epsSU6ud1 = settings->parm("BeamRemnants:epsSU6ud1");
...
bool SoftStrings::setKinematics(int typediff,vector<EventParticle*>& outgoing)
{
  ...
  */
  ...
  if(partonB.size()>0) {
    ...
    if(idRB == 92 ) {
	if(mB < 1.0) {
	    cout << "SoftStrings mB = " << mB << " idRB= "<< idRB << " typeDIff= "<< typeDiff <<endl;
  ...
  // Check consistency: momentum, baryon number...
  Vec4 ptot=0.0;
  int ibar=0;
...
{ 
  ParticleDataEntryPtr pdr =jamParticleData->find(id1);
...
pair< int, int> SoftStrings::splitFlav( int id) 
{
  ...
  // Diagonal mesons: assume complete mixing ddbar and uubar.
   ...
   if (iq2 < 3 || id2 == 33) {
     ...
     if (id == 221 && eCM > 2*MK && rndm->flat() < fracEtass) iq4 = 3;
     if (id == 331 && eCM > 2*MK && rndm->flat() < fracEtaPss) iq4 = 3;
...
pair< int, int> SoftStrings::splitFlav( int id) 
{
  ...
  // Octet baryons.
    ...
    // Use SU(6) weight proton=1/3d(uu)1 + 1/6u(ud)1 + 1/2u(ud)0
    //                 neutron=1/3u(dd)1 + 1/6d(ud)1 + 1/2d(ud)0
      ...
      //double eps1=0.0, eps2=0.0; // exact SU(6)
      //double eps1=1.0, eps2=1.0;
      double uu1=2.0+epsSU6uu1, ud1=1.0+epsSU6ud1; //ud0=3.0;
      if    (iq1 == iq2 && rr6 < uu1) { iq4 = iq3; iq5 = 1100 * iq1 + 3;}
      else if             (rr6 < uu1) { iq4 = iq1; iq5 = 1100 * iq3 + 3;}
      else if (rr6 < uu1+ud1) { iq4 = iq2; iq5 = 1000 * iq1 + 100 * iq3 + 3;}
      else               { iq4 = iq2; iq5 = 1000 * iq1 + 100 * iq3 + 1;}
	...
	vector<EventParticle*>& outgoing)
{
  ...
  ParticleDataEntryPtr pdr =jamParticleData->find(idhad);
...
int SoftStrings::findIdRes(vector<ResolvedPartonQ>& parton,double m)
{
  ...
  for(int i=0;i<ntable;i++) {
    ParticleDataEntryPtr p=table->getParticle(i);
    prob[i]=0.0;
    if(m > p->mMin()) {
     prob[i]=p->spinType();
     total += prob[i];
...
//...Quark contents.
  ...
  //Vec4 ptot=Pmeson;
    ...
    if(kfm !=0) {
	ParticleDataEntryPtr pdr =jamParticleData->find(kfm);
    ...
    p4.py(-pkc12);
      ...
      if(diffra==1) {
	   ...
	   // stringFragment(q4,xoutB, outgoing);
	    ...
	    ParticleDataEntryPtr pdr =jamParticleData->find(id2);
...
//------------------------------------------------------------------------
  ...
  if(isDebug>1) {
	...
	if(event[i].isFinal()) {
	    ...
	    ParticleDataEntryPtr pdr =jamParticleData->find(id);
	    //ParticleDataEntryPtr pdr =&(event[i].particleDataEntry());
	    ...
	    /*
        if(v[0] < abs(v[3])) {
  	cout << "after string fragment t < z t= "<< v[0] << " z= "<< v[3]
	  << " xprod0= " <<xprod[0] << " xprod3= "<< xprod[3]
	  << " xout0= " <<xout[0] << " xout3= "<< xout[3] <<endl;
	exit(1);
        }
	*/
	    pa->setCoordinate(v);
      ...
      vector<pair<int,int>>& q)
{
  ...
  do {
    if(ntry++ > 100) {
      event.list();
      cout << "SoftSting::findConstQuark infinite loop? "<< endl;
      exit(1);
    }
