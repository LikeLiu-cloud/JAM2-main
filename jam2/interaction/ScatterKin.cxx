...
void ScatterKin::initialize(TwoBodyInterList* inter)
{
  ...
  for(int i=0; i<nIn; i++) {
	...
	cout << "scatterKin:ini meff?  meff= "<< meff
	    << " mInEff= "<< mInEff[i]
	    << " meff= "<< meff
    ...
    /*
    if(xOut[i][0] < abs(xOut[i][3])) {
      cout << "ScatterKin::ini t= "<< xOut[i][0]<< " xOut[3]= "<< xOut[i][3]
       << " xIn[0]= " << xIncoming[i][0] << " xIn[3]= "<< xIncoming[i][3]<<endl;
      cout << " ctime= "<< ctime <<" dt= "<< dt << endl;
      cout << "vel= "<< vel;
      exit(1);
    }
    */
