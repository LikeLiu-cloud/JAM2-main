...
private:
  ...
  struct HoldProcess {
    ...
    HoldProcess(shared_ptr<ProcessSelectorHook> hook, int proc, double b = -1.0)
...
public:
    ...
    ~JPythia();
    void init0();
    void init(JamParticleData* jd,HadronDecay* dec,std::string infile);
    bool initBeam(const int id[2], const double m[2],const double m0[2],Vec4& pA, Vec4& pB,
