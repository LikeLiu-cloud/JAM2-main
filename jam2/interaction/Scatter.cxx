...
int Scatter::usePythia(double eCM)
{
  ...
  double emin= eMinPert + 2*max(0.0, scatt->meff(0)-0.938) + 2*max(0.0, scatt->meff(1)-0.938);

  if ( eCM  <= emin ) return 0;
  //if ( exp( -(eCM - eMinPert) / eWidthPert ) > rndm->flat()  ) return 0;
	...
	Collision* event)
{
	...
	// t-channel string formation.
		...
		if(usepythia==1) {
  		  if((success=pythia->initBeam(idIn,meff,m,pc1,pc2,preHadronA,preHadronB)))
		  success=pythia->generate(scatt->nColl(),event->getNColl(),
			constQ[0],constQ[1],xout1,xout2,outgoing);
		} else {
		  if((success=pythia2->initBeam(idIn,meff,m,pc1,pc2,preHadronA,preHadronB)))
    ...
    if(!success) {
      ...
      if(usepythia>0) {
        channel=SOFT;
        scatt->setChannel(SOFT);
	if(softModel==2) success2=scatt->setKinematics(typeDiff,outgoing);
	else success2= scatt->hijingSoft(typeDiff,outgoing);
	success=true;
      }
      if(!success2 || !success) {
        channel=ELASTIC;
