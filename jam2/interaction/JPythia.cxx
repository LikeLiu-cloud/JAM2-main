...
void JPythia::init(JamParticleData* jd, HadronDecay* dec, string initFile)
{
  ...
  pythia->settings.word("MultipartonInteractions:initFile",initFile);

  pythia->readString("LowEnergyQCD:all = on");
  settings->flag("SoftQCD:inelastic", true);
  ...
  //pythia->settings.flag("SoftQCD:nonDiffractive",true);
  //pythia->settings.flag("SoftQCD:singleDiffractive", true);
  //pythia->settings.flag("SoftQCD:doubleDiffractive", true);
  //pythia->settings.flag("SoftQCD:centralDiffractive",true);
  ...
  if(userHook) {
    //auto userHooks = make_shared<selectSASD>();
    //pythia->setUserHooksPtr(userHooks);
    pythia->addUserHooksPtr(selectSASD);
  }

  if(!pythia->init()) {
    cout << "JPythia init failed "<< endl;
    exit(1);
  }

  cout << " MPI= "<< pythia->settings.flag("PartonLevel:MPI")<<endl;
  cout << " ProcessLevel:all= "<< pythia->settings.flag("ProcessLevel:all") <<endl;
  cout << " PartonLevel:all= "<< pythia->settings.flag("PartonLevel:all") <<endl;
  cout << " softqcd= "<< pythia->settings.flag("SoftQCD:all") << endl;
  cout << " elasitc= "<< pythia->settings.flag("SoftQCD:elastic")<<endl;
  cout << " inel= "<< pythia->settings.flag("SoftQCD:inelastic")<<endl;
  cout << "soft= "<< pythia->settings.flag("SoftQCD:nonDiffractive") <<endl;
  cout <<  "sd= "<< pythia->settings.flag("SoftQCD:singleDiffractive") <<endl;
  cout << "xb= "<<pythia->settings.flag("SoftQCD:singleDiffractiveXB") << endl;
  cout << "ax= "<< pythia->settings.flag("SoftQCD:singleDiffractiveAX")<<endl;;
  cout << "dd= "<< pythia->settings.flag("SoftQCD:doubleDiffractive")<<endl;;
  cout << "doCD= "<< pythia->settings.flag("SoftQCD:centralDiffractive")<<endl;;
...
bool JPythia::initBeam(const int id[2], const double m[2], const double m0[2],Vec4& pA, Vec4& pB,
	bool prehadron1, bool prehadron2)
{
    ...
    double mSaveA = particleData->m0(idA);
    double mSaveB = particleData->m0(idB);
    ParticleDataEntryPtr p1=particleData->findParticle(idA);
    ParticleDataEntryPtr p2=particleData->findParticle(idB);
    if(!p1 || !p2) {
      cout << "JPythia::initBeam p1= "<< p1 << " idA= "<< idA;
      cout << " p2= "<< p2 << " idB= "<< idB
      <<endl;
      exit(1);
    }
  ...
  //info.setECM( eCM);
    ...
    if(!pythia->setBeamIDs(idA, idB) ) {
      cout << "Pythia initialization failed idA= "<< idA << " idB= "<< idB <<" ecm= "<< eCM <<endl;
      cout << " alow= "<< pythia->flag("Beams:allowIDAswitch") << endl;
      return false;
    }
    ...
    */
    ...
    // This does not need for pythia8.3
    ...
    //if(p1->isMeson() || p2->isMeson()) {
    //cout << "idA= " << idA << " idB= " << idB << " ecm= " << eCM <<endl;
    //}
...
int JPythia::convertID(int id, int& isBaryon)
{
	...
	if(id4 == 2130) id4=3212;

	//idnew=id4+id0;
	if(id4==1110 || id4==2220 || id4==3330) idnew = id4+4;
	else idnew=id4+2;
...
int JPythia::whichSide(int ip, int& cq)
{
  ...
  BeamParticle &beamB = pythia->beamB;
    ...
    cout << "JPythia::whichSide infinite loop? ip= " << ip <<endl;
    event.list();
    exit(1);
}
	...
	Vec4& xA, Vec4& xB, vector<EventParticle*>& outgoing)
{
  ...
  // no process or elastic
  if(procCode ==0 || procCode ==102 ) {
    cout << "JPythia::generate process is not pQCD type? code= "<< procCode <<endl;
    event.list();
    exit(1);
  }
  // low-energy process
  if(procCode >150) return false;
  ...
  do {
	...
	beamSide[da2]=2;
     if(event.size() !=5) {
       cout << "JPythia::generate np= "<< event.size() <<endl;
       event.list();
       exit(1);
      }
    } else {
 ...
 // form ministring into one hadron.
    ...
    } else if (status1 >=150 && status1 < 160) { // low energy processess
    } else {
