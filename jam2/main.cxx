...
void outputPhaseSpace(ostream& ofs, int iev, int mass, JAM* jam,int opt)
{
  ...
  //std::cout << "Event = " << iev << ": nspec = " << nonpart << ": ntot = " << mass << ": npart = " << npart << ": npartG = " << npartG << ": nmeson  = " << nmeson << ": nbaryon = " << nbaryon << ": mul = " << nv << std::endl;
  double b = jam->impactParameter();
  ofs << "#Event " << iev << setw(12) << nv << setw(8) << ncollG
      << setw(15) << npartG
      << setw(18) << b
      << setw(8) << npart
      << setw(8) << ncoll
...
int main(int argc, char* argv[]) {
  ...
  string beamA = jam->settings->word("Beams:beamA");
  string beamB = jam->settings->word("Beams:beamB");
  int massA= atoi(beamA.c_str());
  int massB= atoi(beamB.c_str());
  int massAB = massA + massB; //jam->initcond()->massA() + jam->initcond()->massB();
  if(outputPhaseData) {
     //ofs.open(outputFname.c_str() + date();
     ofs.open(outputFname.c_str());
     ofs << "#Nevents " << nev << setw(8) << jam->initcond()->ycm()
