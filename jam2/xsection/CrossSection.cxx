...
void CrossSection::selectOutGoingParticle(CollisionPair& cpair)
{
  ...
  double sigel=cpair.getSigmaElastic();
   if(xsig > sig-sigel) {
    ...
    double m3=ma,m4=mb;
    ParticleDataEntryPtr pa3=pd1;
    ParticleDataEntryPtr pa4=pd2;
    ParticleDataEntryPtr pa5=0, pa6=0;
...
//----------------------------------------------------------------------*
    ...
    pd1=jamParticleData->find(idA);
    pd2=jamParticleData->find(idB);
...
{
  int nHeavy=getHeavyQB(idA)+getHeavyQB(idB);

//...In the case of antibaryon-antibaryon collision,
  if(idA < 0 && idB < 0) {  // antib-antib
    anti=true;
    idA=-idA;
    idB=-idB;
    cpair.setAnti();
...
//...Light collision systems.
    int nHeavy=getHeavyQM(idA)+getHeavyQB(idB);
    if(nHeavy != 0) {
	cpair.setXSTotal(20.0);
	cpair.setXSElastic(20.0);
	//return SigmaMBHeavy();
    ...
    XsecTable::additiveQuarkModel(idA,idB,sigTot,sigEl);
