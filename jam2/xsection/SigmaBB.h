...
protected:
    ...
    ParticleDataEntryPtr pd1,pd2, pout[2];
    ...
    std::vector<ParticleDataEntryPtr> NStar[2], DStar[4];
    std::vector<ParticleDataEntryPtr> protonT, neutronT;
    std::vector<ParticleDataEntryPtr> deltamT, delta0T, deltapT, deltappT;
    std::vector<ParticleDataEntryPtr> N1440T, P1440T;
    double sigin[30];
    int mChanel;
    int nMaxRes;
    bool absorptionBB;
...
// 1:n* 2:p* 3:D*- 4:D*0 5:D*+ 6:D*++
    ...
    Pythia8::ParticleDataEntryPtr outParticle(int i) {return pout[i];}
    ...
    std::vector<ParticleDataEntryPtr> pickRTable(int idn, int& idr);
