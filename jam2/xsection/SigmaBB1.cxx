...
// 18,19) s-wave
	...
	sigin[17]=0.5*sig0[9];
        sigin[18]=0.5*sig0[9];
	cpair.setOutGoing(17,sigin[17], 2212,202112);//pn->pn*(1440)
	cpair.setOutGoing(18,sigin[18],202212, 2112);//pn->p*(1440)n
    } else  {
...
//...deuteron + pi+ cross section.
    ...
    if(iz1+iz2==0) {
	 ...
	 if(optSwave==1) {
	   sigin[16]=sig1[9];
	   cpair.setOutGoing(16,sigin[16],2112,202112); // n n(1440)
	 } else {
    ...
    // pp collision
	 ...
	 if(optSwave==1) {
	   sigin[16]=sig1[9];
	   cpair.setOutGoing(16,sigin[16],2212,202212); // p p(1440)
	 } else {
           sigin[16]=sig1[9]/3.0;    // s-wave pp->pp pi0
           sigin[17]=sig1[9]*2.0/3.0; // s-wave pp->pn pi+
...
Pythia8::ParticleDataEntryPtr SigmaBB1::selectDN(int idn, int& idr)
{
   if(idn==2212)  {
       idr=id_nucl;
...
Pythia8::ParticleDataEntryPtr SigmaBB1::selectResonance(ParticleTable* table)
{
    double bwtot=0.0;
    int n=table->size();
    vector<double>  bw(n);
...
{    
  ...
  vector<ParticleDataEntryPtr> table1 = pickRTable(idn1,idv[0]);
  vector<ParticleDataEntryPtr> table2 = pickRTable(idn2,idv[1]);
  int n1=table1.size();
  int n2=table2.size();
  if(n1==0 || n2==0) {
...
//...2 is a nucleon(*) 1 is a delta(*)
    ...
    Pythia8::ParticleDataEntryPtr kcd=pd1;
    double emn=em2;
    double emd=em1;
...
double SigmaBB1::DetBal1(int msel,Pythia8::ParticleDataEntryPtr p,double m1,double m2)
{
...
double SigmaBB1::BW1(int idetsw,Pythia8::ParticleDataEntryPtr p,double em,double mr0,double m2)
{
//...Options.
//...isw:
