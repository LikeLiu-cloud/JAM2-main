...
//....nn/pp collision.
    ...
    if(z1 == z2) {
	  ...
	  // cross section from Pythia8
	  //sigTotPy8->calc(2212,2212,ecm);
...
vector<ParticleDataEntryPtr> SigmaBB::pickRTable(int idn, int& idr)
{
  if(idn==2212)  {
    idr=id_nucl;
    return protonT;
...
double SigmaBB::sigmaS1(int ipair,double ecm,int iz01,int iz02)
{
    int iz1=iz01;
    int iz2=iz02;
    if(id1 == id_nucl) {
...
//...Get total and elastic cross sections.
    //if(isigt != 0) sigTot=XsecTable::sigBBS1(isigt,ecm);
    sigTot=XsecTable::sigBBS1(isigt,ecm);
    if(isige >= 1) sigEl=XsecTable::sigBBS1(isige,ecm);
    else sigEl=sigTot;
...
double SigmaBB::sigmaS2(int ipair,double ecm,int iz1,int iz2)
{
  //int isw=0;
...
//...Get total and elastic cross sections.
  //if(isigt>0) sigTot=XsecTable::sigBBS2(isigt,ecm);
  sigTot=XsecTable::sigBBS2(isigt,ecm);
  if(isige >= 1) sigEl=XsecTable::sigBBS2(isige,ecm);
  else sigEl=sigTot;
  return thr;
