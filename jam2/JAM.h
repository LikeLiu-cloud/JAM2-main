...
private:
    ...
    double aLund,bLund,mesonUDL1S0J1, mesonSL1S0J1,popcornRate, probQQtoQ, probStoUD, ecmPow;
    std::string dataPath={};
...
public:
  ...
  JAM(std::string xmlDir = "../share/Pythia8/xmldoc", bool printBanner = false);
  //: Pythia(xmlDir,printBanner) {;}

  ~JAM();

  // Set default parameters.
  void jamDefaultParam();

  // Read in commands from external file.
  bool readFile(std::string fname) { 
    cout << "# input file name= " << fname << endl;
    return pythia->readFile(fname);
  }
